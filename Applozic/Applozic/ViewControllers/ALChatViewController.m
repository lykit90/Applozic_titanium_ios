...
@property (weak, nonatomic) IBOutlet UILabel *jobSal;
...
//    }
    ...
    CGFloat COORDINATE_POINT_Y = titleLabelButton.frame.size.height - 19;
...
-(void)setButtonTitle
{
    ALChannelService *channelService = [[ALChannelService alloc] init];
    self.alChannel = [channelService getChannelByKey:self.channelKey];
    [titleLabelButton setTitle:self.alChannel.metadata[@"coydispm"] forState:UIControlStateNormal];
    
    // Set job detials
    self.coyImg.layer.borderWidth = 1;
    self.coyImg.layer.cornerRadius = 5;
    self.coyImg.layer.borderColor = [UIColor lightGrayColor].CGColor;
    [self.coyImg setImageWithURL:self.alChannel.metadata[@"logo"]];
    
    [self.jobTtl setText:self.alChannel.name];
    [self.jobLoc setText:self.alChannel.metadata[@"loc"]];
    
    NSString *sal = [NSString stringWithFormat:@"$%@ %@", self.alChannel.metadata[@"salmin"], self.alChannel.metadata[@"salperiod"]];
    if ( self.alChannel.metadata[@"salindi"] ) {
        sal = [NSString stringWithFormat:@"%@ $%@ %@", self.alChannel.metadata[@"salindi"], self.alChannel.metadata[@"salmin"], self.alChannel.metadata[@"salperiod"]];
    }
    [self.jobSal setText: sal];
...
-(void)updateLastSeenAtStatus: (ALUserDetail *) alUserDetail
{
    ...
    if(self.channelKey != nil)
    {
        ...
        {
            // [self.label setText:@"tap to view employer's available jobs"]; // [channelService stringFromChannelUserList:self.channelKey]
        }
    }
    else if (value > 0)
    {
